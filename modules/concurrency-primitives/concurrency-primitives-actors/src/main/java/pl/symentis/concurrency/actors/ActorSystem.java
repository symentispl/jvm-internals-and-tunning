...
class ActorSystem {
  ...
  void start() {
    ...
    for (int i = 0; i < availableProcessors; i++) {
      executorService.submit(() -> {
        while (running) {
          try {
            SchedulableActorRef<? extends Actor> ref = actors.poll(1, TimeUnit.SECONDS);
            if (ref != null) {
              try {
                ref.onSchedule();
              } finally {
  ...
  <T extends Actor> ActorRef<T> spawn(Class<T> clazz) {
    try {
       SchedulableActorRef<T> actorRef = new SchedulableActorRef<T>(clazz.getConstructor().newInstance());
      // schedule actor
