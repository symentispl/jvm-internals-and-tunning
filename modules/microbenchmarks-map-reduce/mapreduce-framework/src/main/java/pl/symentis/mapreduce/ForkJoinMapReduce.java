...
public class ForkJoinMapReduce implements MapReduce {
	...
	@Override
	public <In, MK, MV, RV> void run(Input<In> input, MapReduceJob<In, MK, MV, RV> mapReduceJob,
			Output<MK, RV> output) {

		Map<MK, List<MV>> map = pool.invoke(new InputTask<In, MK, MV>(input, mapReduceJob.mapper()));

		for (Map.Entry<MK, List<MV>> entry : map.entrySet()) {
			output.emit(entry.getKey(),
					mapReduceJob.reducer().reduce(entry.getKey(), () -> entry.getValue().iterator()));
		}
