 ...
 * 
 * <ul>
 * <li>block start marker - 4 bytes, should we equal to {@code Integer.MAX_VALUE}, this helps us to distinguish new block (marker will be equal to 0)</li>
 ...
 *
 */
public class Block implements Iterable<PersistentRecord> {
	 ...
	 * @return
	 */
	public static Block create(int blockNr, int blockSize) {
		ByteBuffer buffer = ByteBuffer.allocate(blockSize);
		// write block header
		buffer.putInt(Integer.MAX_VALUE);
		buffer.putInt(blockSize - BLOCK_HEADER_SIZE);
		return new Block(blockNr, buffer);
	}
	...
	Block(int blockNr, ByteBuffer buffer) {
		super();
	...
	public Cursor cursor() {
		return new Cursor(buffer.asReadOnlyBuffer().position(BLOCK_HEADER_SIZE));
	}
	...
	public class Cursor implements Iterator<PersistentRecord> {

		private final ByteBuffer roBuffer;
		private boolean hasNext;
		private PersistentRecord record;

		public Cursor(ByteBuffer roBuffer) {
