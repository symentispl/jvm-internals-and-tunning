...
public class BlockFile {
	...
	public BlockFile(FileChannel fileChannel, int blockSize) {
		super();
	...
	public Block read(int blockNumber, Supplier<ByteBuffer> byteBufferSupplier) throws IOException {
		...
		if ( byteBuffer.remaining() == blockSize ) {
			...
			byteBuffer.rewind();
			return Block.read( blockNumber, blockSize, byteBuffer);
		} else {
			throw new IllegalStateException(format("buffer %s doesn't have enough bytes, expected $d", byteBuffer, blockSize));
		}
	}
	...
	public void write(int blockNumber, ByteBuffer byteBuffer) throws IOException {
		...
		if ( byteBuffer.remaining() == blockSize ) {
			...
			throw new IllegalArgumentException(format("buffer %s doesn't have enough bytes, expected $d", byteBuffer, blockSize));
		}
	}
