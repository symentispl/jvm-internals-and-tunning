...
REVEAL_JS_URL = "https://github.com/hakimel/reveal.js/archive/%s.zip" % \
                REVEAL_JS_VERSION
...
def install_asciidoctor(logger):
    ...
    if "asciidoctor-revealjs" not in packages:
        logger.error("asciidoctor-revealjs not installed"
                     ", execute `gem install asciidoctor-revealjs`")
        sys.exit(1)
